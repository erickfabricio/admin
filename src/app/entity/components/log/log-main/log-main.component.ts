...
export class LogMainComponent implements OnInit {
  ...
  @ViewChild("list", { static: true }) list: LogListComponent;
  @ViewChild("tabCrud", { static: true }) tabCrud;
  @ViewChild("crud", { static: true }) crud: LogCrudComponent;
  view: string;
  ...
  captureEventList() {
    this.list.eventCrud.pipe().subscribe(data => {
      ...
      this.crud.log = data.log;
      this.crud.show();

      //Change and enable tag      
      this.tabCrud.textLabel = "Log";
